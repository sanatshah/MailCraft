...
describe('App', () => {
  ...
  it('renders dashboard KPIs when analytics data exists', async () => {
    ...
    expect(screen.getByTestId('home-trend-chart')).toHaveAttribute('aria-hidden', 'true')
  ...
  it('keeps period control before Manage templates in DOM order', async () => {
    render(<App />)
    await waitFor(() => {
      expect(screen.getByTestId('home-dashboard')).toBeInTheDocument()
    })
    ...
    const manageTemplatesLink = screen.getByRole('link', { name: 'Manage templates' })
    expect(
      periodSelect.compareDocumentPosition(manageTemplatesLink) & Node.DOCUMENT_POSITION_FOLLOWING,
    ).toBeTruthy()
  })

...
function setupFetch(options: {
  ...
  createdTemplate?: unknown
  templateById?: unknown
} = {}) {
  ...
  const templateById = options.templateById ?? createdTemplate
  ...
  vi.stubGlobal(
    ...
    vi.fn((input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (path === '/api/templates/new-template-id') {
        return Promise.resolve(new Response(JSON.stringify(templateById)))
      }
...
describe('App', () => {
  ...
  it('opens editor when creating a template from templates list', async () => {
    render(<App />)
    await waitFor(() => {
      expect(screen.getByTestId('home-dashboard')).toBeInTheDocument()
    })

...
function formatTrendTick(date: string): string {
...
export function Home() {
  ...
  const hasTrendSeries = (trends?.series.length ?? 0) > 0
    ...
    <div className="home-dashboard" data-testid="home-dashboard">
      <header className="home-header">
        <div className="home-header-copy">
          <h1 className="home-page-title">Email performance</h1>
          <p className="home-page-subtitle">
            Track delivery health, failures, and engagement for the selected period.
          </p>
        </div>
      ...
      {isEmpty && (
        <div className="home-empty-banner" data-testid="home-empty-banner">
          ...
          <p>
            Start by sending a campaign or test email. Dashboard metrics appear once your send
            pipeline posts events to <code className="home-code">POST /api/events/message</code> and
            recipients load the open tracking pixel.
          </p>
        </div>
      )}
      ...
      {trends && (
        <section className="home-section" aria-labelledby="home-trends-heading">
          ...
          <p className="home-section-description">
            Visual trend bars are paired with a screen-reader table that lists exact daily values.
          </p>
          {hasTrendSeries ? (
            <>
                  ...
                  {trends.series.map((day) => {
                    ...
                    return (
                      <div key={day.date} className="home-trend-col" title={day.date}>
                        ...
                        <span className="home-trend-day">{formatTrendTick(day.date)}</span>
                      </div>
                    )
                  })}
                </div>
              </div>
              <div className="home-visually-hidden">
                <table
                  data-testid="home-trend-table"
                  aria-label={`Daily activity values for the last ${trends.period_days} days`}
                >
                  <thead>
                    <tr>
                  ...
                  <tbody>
                    {trends.series.map((day) => (
                      <tr key={`row-${day.date}`}>
                        <th scope="row">{formatTrendDateLabel(day.date)}</th>
                        <td>{day.sent}</td>
                        <td>{day.failed}</td>
                        <td>{day.opens}</td>
                      </tr>
            ...
            <p className="home-empty-note" data-testid="home-trend-empty">
              No daily activity has been recorded in this period yet.
            </p>
          )}
        </section>
      )}
        ...
        {topTemplates && (
          <section className="home-section" aria-labelledby="home-top-heading">
            ...
            {topTemplates.templates.length > 0 ? (
              ...
              <p className="home-empty-note" data-testid="home-top-empty">
                No template activity in this period yet.
              </p>
            )}
          </section>
        )}
        ...
        {overviewData && (
          <section className="home-section" aria-labelledby="home-fail-heading">
            ...
            {overviewData.recent_failures.length > 0 ? (
              ...
              <p className="home-empty-note" data-testid="home-fail-empty">
                No delivery failures were recorded for this period.
              </p>
            )}
          </section>
        )}
